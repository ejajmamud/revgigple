...
<html lang="{{ app()->getLocale() }}" dir="{{ config()->get('direction') }}" @class(['dark' => current_theme() === 'dark'])>
    ...
    <head>
        ...
        <style>
            ...
            .auth-login-registration{
                ...
                background: linear-gradient(120deg, rgba(249,240,130,1) 0%, rgba(239,237,134,1) 16%, rgba(214,232,147,1) 36%, rgba(172,222,168,1) 60%, rgba(114,209,197,1) 82%, rgba(42,193,233,1) 100%) !important;
                
                /*background: url("/../public/storage/about-us/bg-shap.png") !important ;*/
    ...
    <body class="antialiased bg-gray-50 dark:bg-[#161616] text-gray-600 min-h-full flex flex-col application application-ltr overflow-x-hidden {{ app()->getLocale() === 'ar' ? 'application-ar' : '' }}">
        ...
        {{-- Container --}}
        <div id="auth-login" class="min-h-screen flex grow bg-slate-50 dark:bg-zinc-700" style="display: flex;">
                    <!--<div id="auth-login" class="min-h-screen flex grow bg-slate-50 dark:bg-zinc-700" style="display: flex; flex-direction: row-reverse !important; ">-->
            
            
            <div  class="hidden w-full place-items-center lg:grid auth-login-registration" >
                
            {{-- LetStartedComponent --}}
                    
         
              @livewire('main.let-started.let-started-component')
                
                <!--<div style="display:none" id="auth-login-one" >-->
                    
                    
                <!--    <img style="display:none" src="/../public/storage/about-us/bg-shap.png">-->
                <!--</div>-->
                <div id="auth-login-two" style="display:none" class="w-full px-2 py-40 sm:py-48 sm:px-12 flex flex-col justify-center relative bg-no-repeat bg-center bg-cover h-full" @if(settings('auth')->wallpaper) style="background-image: url({{ src(settings('auth')->wallpaper) }})" @endif>
            ...
            <main class="flex w-full flex-col items-center bg-white dark:bg-zinc-800 lg:max-w-md xl:max-w-xl" >
                @yield('content')
            </main>
        </div>
...
//     echo '<script>
//       document.getElementById("auth-login-one").style.display = "block";
//       document.getElementById("auth-login-two").style.display = "none";
//         document.getElementById("lets-started-two").style.display = "block";
//       document.getElementById("lets-started-one").style.display = "none";
...
<!--- <script>
    ...
    //     document.getElementById('auth-login-two')?.style.display = 'none';
    // } else {
    //     document.getElementById('auth-login-one')?.style.display = 'none';
    //     document.getElementById('auth-login-two')?.style.display = 'block';  
    // }
</script>-->
