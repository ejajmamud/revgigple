        ...
        <div class="col-span-12 lg:col-span-4">
            ...
            <div class="flex flex-col text-center bg-white dark:bg-zinc-800 rounded-md shadow-sm mb-6">
                ...
                <div class="flex-1 flex flex-col p-8">
                    ...
                    <!--<div class="relative rounded-full overflow-hidden flex-shrink-0 mx-auto" wire:ignore>-->
                    <!--    <img id="profile-avatar-preview" class="relative rounded-full w-28 h-28 object-cover lazy" src="{{ src( settings('appearance')->placeholder ) }}" data-src="{{ src( settings('appearance')->placeholder ) }}" alt="{{ auth()->user()->username }}">-->
                    <!--    <label for="profile-avatar-container" class="absolute inset-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center text-sm font-medium text-white opacity-0 hover:opacity-100">-->
                    <!--        <span>{{ __('messages.t_change') }}</span>-->
                    <!--        <input type="file" id="profile-avatar-container" wire:model="avatar" @change="avatar" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer border-gray-300 rounded-md">-->
                    <!--    </label>-->
...
/*        </style>          
  ...
  <!--anam code -->
   ...
   @if (auth()->user()->isAvatar == true)
    {{-- Display this when isAvatar is true --}}
    {{-- Avatar --}}
<div class="relative rounded-full overflow-hidden flex-shrink-0 mx-auto" wire:ignore>
    <img id="profile-avatar-preview" class="relative rounded-full w-28 h-28 object-cover lazy" src="{{ placeholder_img() }}" data-src="{{ src(auth()->user()->avatar) }}" alt="{{ auth()->user()->username }}">
    <label for="profile-avatar-container" class="absolute inset-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center text-sm font-medium text-white opacity-0 hover:opacity-100">
...
@else
    {{-- Display this when isAvatar is not true --}}
    <div class="relative rounded-full overflow-hidden flex-shrink-0 mx-auto" wire:ignore>
        <img id="profile-avatar-preview" class="relative rounded-full w-28 h-28 object-cover lazy" src="{{ src( settings('appearance')->placeholder ) }}" data-src="{{ src( settings('appearance')->placeholder ) }}" alt="{{ auth()->user()->username }}">
    </div>
@endif
...
<div class="flex flex-wrap justify-center items-center space-x-2">
    <span class="mb-2 text-xs">Show login users profile</span>
    <label class="mb-2 text-xs">
        <input type="radio" wire:model="isAvatar" name="userAvatar" value="true" wire:click="showProfile" x-on:click="reloadPage()" id="showProfile" {{ $isAvatar == 'true' ? 'checked' : '' }}>
        <span class="ml-1">Show Profile</span>
    </label>
    <label class="mb-2 text-xs">
        <input type="radio" wire:model="isAvatar" name="userAvatar" value="false" wire:click="showAvatar" x-on:click="reloadPage()" id="showAvatar" {{ $isAvatar == 'false' ? 'checked' : '' }}>
        <span class="ml-1">Show Avatar</span>
    </label>
</div>

<script>
    function reloadPage() {
        // Trigger a custom event to reload the page
        window.dispatchEvent(new Event('reload-page'));
    }

    // Listen for the custom event to trigger a page reload
...
<div class="flex flex-wrap justify-center items-center space-x-2">
    <span class="mb-2 text-xs">Show profile to guest</span>
    <label class="mb-2 text-xs">
        <input type="radio" wire:model="isguestAvatar" name="guestAvatar" value="true" wire:click="showlogoutProfile" x-on:click="reloadPagetwo()" id="showProfileGuest" {{ $isguestAvatar == 'true' ? 'checked' : '' }}>
        <span class="ml-1">Show Profile</span>
    </label>
    <label class="mb-2 text-xs">
        <input type="radio" wire:model="isguestAvatar" name="guestAvatar" value="false" wire:click="showlogoutAvatar" x-on:click="reloadPagetwo()" id="showAvatarGuest" {{ $isguestAvatar == 'false' ? 'checked' : '' }}>
        <span class="ml-1">Show Avatar</span>
    </label>
</div>

<script>
    function reloadPagetwo() {
        // Trigger a custom event to reload the page
        window.dispatchEvent(new Event('reload-page-two'));
    }

    // Listen for the custom event to trigger a page reload
    window.addEventListener('reload-page-two', function () {
        location.reload();
    });
</script>
...
<!--        const isAvatar = localStorage.getItem("isAvatar");-->
<!--        if (isAvatar !== null) {-->
<!--            document.querySelector(`input[value="${isAvatar}"]`).checked = true;-->
<!--        }-->
<!--    });-->

<!--    document.querySelectorAll('input[type="radio"]').forEach(function (input) {-->
<!--        input.addEventListener("click", function () {-->
<!--            localStorage.setItem("isAvatar", this.value);-->
<!--        });-->
<!--    });-->
<!--</script>-->
...
<!--<script>-->
<!--    document.addEventListener("DOMContentLoaded", function () {-->
<!--        const isguestAvatar = localStorage.getItem("isguestAvatar");-->
<!--        if (isguestAvatar!== null) {-->
<!--            document.querySelector(`input[value="${isguestAvatar}"]`).checked = true;-->
<!--        }-->
<!--    });-->

<!--    document.querySelectorAll('input[type="radio"]').forEach(function (input) {-->
<!--        input.addEventListener("click", function () {-->
<!--            localStorage.setItem("isguestAvatar", this.value);-->
<!--        });-->
<!--    });-->
<!--</script>-->
...
@endpush
  ...
  <script>
    // Function to store the selected values in localStorage for the guest section
    function storeGuestSectionData() {
        const isguestAvatar = document.querySelector('input[name="guestAvatar"]:checked').value;
        localStorage.setItem("guestAvatar", isguestAvatar);
    }

    // Function to store the selected values in localStorage for the user section
    function storeUserSectionData() {
        const isAvatar = document.querySelector('input[name="userAvatar"]:checked').value;
        localStorage.setItem("userAvatar", isAvatar);
    }

    // Function to retrieve and set the stored values when the page loads
    function loadStoredData() {
        const guestAvatar = localStorage.getItem("guestAvatar");
        if (guestAvatar !== null) {
            document.querySelector(`input[name="guestAvatar"][value="${guestAvatar}"]`).checked = true;
        }

        const userAvatar = localStorage.getItem("userAvatar");
        if (userAvatar !== null) {
            document.querySelector(`input[name="userAvatar"][value="${userAvatar}"]`).checked = true;
        }
    }
    ...
    // Add event listeners to radio buttons to store data when they are clicked
    document.querySelectorAll('input[name="guestAvatar"]').forEach(function (input) {
        input.addEventListener("click", storeGuestSectionData);
    });

    document.querySelectorAll('input[name="userAvatar"]').forEach(function (input) {
        input.addEventListener("click", storeUserSectionData);
    });
</script>
