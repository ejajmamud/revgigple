...
@if (request()->is('/'))
    ...
    <div class="home-hero-section bg-red-700" style="display:none;">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center">
            <div class="w-full md:max-w-lg">
                ...
                <h1 class="text-center sm:ltr:text-left sm:rtl:text-right mt-4 text-xl tracking-tight font-extrabold text-white sm:mt-5 sm:text-3xl lg:mt-6 xl:text-4xl"
                    ...
                    {{ __('messages.t_find_best') }} {{ __('messages.t_freelance') }}
                    <br> {{ __('messages.t_services_for_ur_business') }}
                </h1>
    ...
    <div class="hero_area">
        <div class="container">
            <div class="row">
                <div class="col-lg-8">
                    <img src="{{ asset('/public/storage/home/img1.png') }}" alt="not" class="w-100 hero_img">
                </div>
                <div class="col-lg-4">
                    <div class="hero_content">
                        <h2>Find the right talent</h2>
                        <p>Gigple is one of South East Asia's fastest growing platforms. Focused on Marketing & Communications, Gigple helps talent connect with high quality freelance work</p>
                        <form action="{{ url('search') }}" accept="GET">
...
{{-- Content --}}
<!--        <main class="flex-grow" style="background: -o-linear-gradient(330deg, rgba(249,240,130,1) 0%, rgba(239,237,134,1) 16%, rgba(214,232,147,1) 36%, rgba(172,222,168,1) 60%, rgba(114,209,197,1) 82%, rgba(42,193,233,1) 100%);-->
...
<main class="flex-grow" id="lets-started">
    <div class="container pb-4 pt-4">
        @yield('content')
    </div>
    

{{--    <div id="about-us" class="" style="display:none">--}}
{{--        @yield('content')--}}
{{--    </div>--}}
{{--    <div id="about-us-one" style="display:block"--}}
{{--         class="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 pt-16 pb-24 space-y-8 min-h-screen">--}}
{{--        @yield('content')--}}
{{--    </div>--}}
...
<script>
    ...
    if (window.location.href === "{{ url('about-us') }}") {
        ...
        document.getElementById('about-us-one').style.display = 'none';
    } else if (window.location.href === "{{ url('giggers') }}") {
        document.getElementById('about-us').style.display = 'block';
        document.getElementById('about-us-one').style.display = 'none';
    }
    
    
    else if (window.location.href === "{{ url('lets-started') }}") {
        document.getElementById('about-us').style.display = 'block';
        document.getElementById('about-us-one').style.display = 'none';
    }
    
     else if (window.location.href === "{{ url('business') }}") {
        document.getElementById('about-us').style.display = 'block';
        document.getElementById('about-us-one').style.display = 'none';
    }
    
      else if (window.location.href === "{{ url('business/login') }}") {
        document.getElementById('about-us').style.display = 'block';
        document.getElementById('about-us-one').style.display = 'none';
    }


   else if (window.location.href === "{{ url('business/signup') }}") {
        document.getElementById('about-us').style.display = 'block';
        document.getElementById('about-us-one').style.display = 'none';
    }
    
       else if (window.location.href === "{{ url('business/dashshboard') }}") {
        document.getElementById('about-us').style.display = 'block';
        document.getElementById('about-us-one').style.display = 'none';
    }
  ...
  const mainElement = document.getElementById('lets-started');
  mainElement.style.background = "linear-gradient(120deg, rgba(249,240,130,1) 0%, rgba(239,237,134,1) 16%, rgba(214,232,147,1) 36%, rgba(172,222,168,1) 60%, rgba(114,209,197,1) 82%, rgba(42,193,233,1) 100%)";
}
...
<script>
     ...
     if(window.location.href === "{{ url('business/login') }}") {

  const mainElement = document.getElementById('lets-started');

  mainElement.style.background = "linear-gradient(120deg, rgba(249,240,130,1) 0%, rgba(239,237,134,1) 16%, rgba(214,232,147,1) 36%, rgba(172,222,168,1) 60%, rgba(114,209,197,1) 82%, rgba(42,193,233,1) 100%)";

  const container = mainElement.querySelector('.container.pb-4.pt-4');

   container.className = container.className.replace('container pb-4 pt-4', '');
...
else if(window.location.href === "{{ url('business/dashboard') }}") {
