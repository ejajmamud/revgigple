...
@if (auth()->check())
    @if ($isAvatar)
        {{-- Display this when the user is authenticated and isAvatar is true (profile image) --}}
        <div class="flex-shrink-0 mb-5 md:mb-0">
            <div class="inline-block relative">
                <img src="{{ placeholder_img() }}" data-src="{{ src($user->avatar) }}" alt="{{ $user->username }}" class="inline-block w-16 h-16 rounded-full object-cover lazy">
            </div>
    ...
    @else
        {{-- Display this when the user is authenticated and isAvatar is false (avatar image) --}}
        <div class="flex-shrink-0 mb-5 md:mb-0">
            <div class="inline-block relative">
                <img src="{{ src(settings('appearance')->placeholder) }}" data-src="{{ src(settings('appearance')->placeholder) }}" alt="{{ $user->username }}" class="inline-block w-16 h-16 rounded-full object-cover lazy">
            </div>
        </div>
    @endif
@else
    {{-- Display this when the user is not authenticated --}}
    @if($isguestAvatar)
        {{-- Display this when isguestAvatar is true (profile image) --}}
        <div class="flex-shrink-0 mb-5 md:mb-0">
            <div class="inline-block relative">
                <img src="{{ placeholder_img() }}" data-src="{{ src($user->avatar) }}" alt="{{ $user->username }}" class="inline-block w-16 h-16 rounded-full object-cover lazy">
            </div>
    ...
    @else
        {{-- Display this when isguestAvatar is false (avatar image) --}}
        <div class="flex-shrink-0 mb-5 md:mb-0">
            <div class="inline-block relative">
                <img src="{{ src(settings('appearance')->placeholder) }}" data-src="{{ src(settings('appearance')->placeholder) }}" alt="{{ $user->username }}" class="inline-block w-16 h-16 rounded-full object-cover lazy">
            </div>
...
@endif
    ...
    filter: blur(4px); /* Adjust the blur amount as needed */
                            ...
                            <div class="flex items-center mb-2 justify-center md:justify-normal @if (!auth()->check()) blur-text @endif">
